...
- Optional Test Mode stores bounded local troubleshooting logs for background and content-runtime events, including action toggles, runtime lifecycle, scans, mutation flushes, cache checks, provider requests, translation diagnostics, dictionary lookups, and vocabulary records. Log details are sanitized before storage to redact API keys, authorization headers, tokens, secrets, and passwords; full translation request text is not intentionally logged.
