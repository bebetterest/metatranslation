...
- Unit tests now cover strict whole-block rejection for any invalid alignment, translated-part output, repeated target text, non-contiguous source spans, dictionary provider URL/result parsing, dictionary language/casing fallback and all-language ranking, generic `reasoning: { effort: "none" }` request bodies, structured-output `json_schema`, OpenRouter headers, fenced/think JSON extraction, parse-failure strict retry recovery, settings normalization, alignment coverage diagnostics, fine-grained provider-output failure counts, CSV escaping, and i18n locale completeness.
