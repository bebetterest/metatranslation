...
- Source-side hover dictionary popup with configurable `WiktApi`, `FreeDictionaryAPI`, or `Off` provider and a configurable popup keep-alive window that defaults to `1000ms`. The WiktApi edition remains an internal storage-compatible field, is no longer exposed in Options, and normalizes to the default English edition. Dictionary lookups run in the background service worker, preserve lookup-word casing before lowercase fallback, query Latin words in English before all-language fallback, rank all-language results by likely source language without hard-filtering on page language hints, are cached in IndexedDB, and expose definitions, pronunciations, examples, translations, attribution, and source links to the content tooltip.
