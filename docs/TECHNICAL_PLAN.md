...
- Test Mode log storage and redaction in `src/background/testLogs.ts`.
...
- Local Ollama on the default `http://localhost:11434` family of endpoints is supported by a best-effort session-scoped `declarativeNetRequest` rule that removes the `Origin` header only from extension background requests, avoiding Ollama's default rejection of `chrome-extension://...` origins without affecting remote providers. The default local Ollama root resolves to `/v1/chat/completions`; other provider roots still append `/chat/completions`.
...
- The provider prompt is intentionally short: task, output shape, rules, format-only examples, page metadata, and payload. It avoids literal placeholder block ids so small local models do not copy them instead of payload ids. It explicitly says payload text, adjacent context, and page URL are untrusted webpage data, must not be followed as instructions, and that adjacent context is for disambiguation only and must not be translated.
...
- Source/target hover highlighting through overlay rectangles. Source-side pointer hit-testing uses the local source-span table for word-level hover, while target-side hover still uses validated model alignment.
- Source-side hover dictionary popup with configurable `WiktApi`, `FreeDictionaryAPI`, or `Off` provider and a configurable popup keep-alive window that defaults to `1000ms`. Source dictionary lookup and recording use the locally segmented source word under the pointer, even when model alignment groups multiple source spans into one target part. The WiktApi edition remains an internal storage-compatible field, is no longer exposed in Options, and normalizes to the default English edition. Dictionary lookups run in the background service worker, preserve lookup-word casing before lowercase fallback, query Latin words in English before all-language fallback, rank all-language results by likely source language without hard-filtering on page language hints, are cached in IndexedDB, and expose definitions, pronunciations, examples, translations, attribution, and source links to the content tooltip.
...
- Optional Test Mode stores bounded local troubleshooting logs for background and content-runtime events, including action toggles, runtime lifecycle, scans, mutation flushes, cache checks, provider requests, translation diagnostics, dictionary lookups, and vocabulary records. Provider request attempts also store the full provider request body, full provider response body, and extracted model message text so model-output issues can be inspected. Log details are sanitized before storage to redact API keys, authorization headers, tokens, secrets, and passwords.
- Geometry-based hover support for `input[type=button|submit|reset]`.
- 2-second source-word hover vocabulary recording.
...
- Options page settings, localized helper text, Test Logs refresh/export/clear controls, records search/sort, and UTF-8 BOM CSV export with formula-prefix neutralization.
...
- Unit tests now cover strict whole-block rejection for any invalid alignment, translated-part output, repeated target text, non-contiguous source spans, dictionary provider URL/result parsing, dictionary language/casing fallback and all-language ranking, generic `reasoning: { effort: "none" }` request bodies, structured-output `json_schema`, OpenRouter headers, local Ollama URL resolution and Origin-removal rule scoping, fenced/think JSON extraction, parse-failure strict retry recovery, settings normalization, alignment coverage diagnostics, fine-grained provider-output failure counts, Test Mode log sanitization and retention, CSV escaping, and i18n locale completeness.
...
- `npm run package:zip` produces `artifacts/metatranslation-0.1.3.zip`.
...
- The local Ollama Origin bypass is intentionally limited to background requests for `http://localhost`, `http://127.0.0.1`, `http://0.0.0.0`, and `http://[::1]` on port `11434`. Non-default Ollama deployments should use Ollama's `OLLAMA_ORIGINS` setting.
...
- Use `requestChunkSize`, `requestConcurrency`, `contextWindowChars`, `translationRetryCount`, `tolerantProviderOutput`, and `testMode` settings so users can tune latency, provider rate-limit behavior, adjacent context length, retry behavior, whether imperfect provider output should be recovered, and whether local troubleshooting logs should be written.
- Store Test Mode logs in bounded `chrome.storage.local` records instead of IndexedDB because they are short-lived diagnostic events and should be easy to query, export, and clear from the options page.
