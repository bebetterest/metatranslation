...
try {
  ...
  if (!Array.isArray(directTranslation.blocks) || directTranslation.blocks.length === 0) {
    const rawDirectResponse = await fetchRawModelResponse(optionsClient, {
      baseUrl,
      apiKey,
      model,
      targetLang,
    ...
    console.log(
      `[e2e] raw single-block response: ${JSON.stringify({
        status: rawDirectResponse.status,
        contentLength: rawDirectResponse.content.length,
        contentPreview: rawDirectResponse.content.slice(-2000),
      })}`,
    );
...
async function fetchRawModelResponse(
  optionsClient,
  { baseUrl, apiKey, model, targetLang, sourceLang, pageUrl, blocks },
) {
  const promptBlocks = JSON.stringify(buildE2EPromptBlocks(blocks));
  const promptExamples = JSON.stringify(buildE2EPromptExamples());
  const targetLanguageDescription = describeE2ETargetLanguage(targetLang);

  return parseMaybeJson(
    await optionsClient.evaluate(`
      (async () => {
        const response = await fetch(${JSON.stringify(`${baseUrl.replace(/\/+$/, '')}/chat/completions`)}, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            messages: [
                  { role: 'system', content: 'You translate source blocks and align translated parts to source spans. Return JSON only.' },
                  {
                role: 'user',
                content: [
                  'Task: translate Payload blocks into the configured target language.',
                  'Configured target language: ${targetLanguageDescription}',
                  'Output JSON:',
                  ...
                  'Examples: ${promptExamples.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}',
                  'Page URL: ${pageUrl}',
                  'Source language hint: ${sourceLang ?? 'auto-detect'}',
                  'Payload: ${promptBlocks.replace(/\\/g, '\\\\').replace(/'/g, "\\'")}',
                ].join('\\n'),
              },
            ],
          }),
...
function describeE2ETargetLanguage(targetLang) {
  const trimmed = String(targetLang).trim();
...
function buildE2EPromptBlocks(blocks) {
  return blocks.map((block) => ({
    ...block,
    sourceSpans: buildE2ESourceSpans(block.text).map(({ id, text }) => ({ id, text })),
  }));
}

function buildE2EPromptExamples() {
  return [
    {
...
function buildE2ESourceSpans(text) {
  const spans = [];
  const pattern = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]|[\p{L}\p{N}]+(?:['’_-][\p{L}\p{N}]+)*/gu;
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    const value = match[0];
    if (match.index === undefined || !value.trim()) {
      continue;
    }
  ...
  return [{
    ...
    end: fallback.index + fallback[0].length,
    text: fallback[0],
  }];
}
